...
pg_dump_structure:
	...
	docker cp polestar_affinity_postgres:/var/lib/postgresql/structure.sql .
...
pg_dump:
	...
	docker cp polestar_affinity_postgres:/var/lib/postgresql/$(file) .
	docker compose exec postgres sh -c "rm /var/lib/postgresql/$(file)"

pg_dump_restore:
	@make -i dropdb db=$(db)
	make createdb db=$(db)
	docker cp $(file) polestar_affinity_postgres:/var/lib/postgresql/$(file)
...
pg_get_remote_db:
	...
	docker cp dump.psql customer_success_portal_postgres:/var/lib/postgresql/dump.psql
